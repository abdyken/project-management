...
| US1-2 | Keyword search               | `GET /api/programs?q=<word from a title>` in original, lower and upper case                   | 200; exactly the programs whose title or faculty contains the word; same result for all cases | Typing the keyword filters the list                       |
